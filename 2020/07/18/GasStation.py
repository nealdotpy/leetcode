...
def can_complete_circuit(gas, cost) -> int:
	...
	# def breadth_first_search(tank_vol: int, gas, cost):
	# 	if tank_vol - cost[0] < 0:
	# 		return 


	# for i in range(len(gas)):
	# 	if cost[i] < gas[i]:
	# 		path_left_to_travel = gas[i+1:] + gas[:i]
	# 		cost_left_to_travel = cost[i+1:] + cost[:i]
	# 		breadth_first_search(gas[i] - cost[i], path_left_to_travel, cost_left_to_travel)
