...
def findOrder(numCourses: int, prerequisites):
	...
	while free:
		...
		for successor_course in successor_courses_by_course[free_course]:
			...
			prereq_count_by_course[successor_course] or free.add(successor_course) # 0 or anything -> anything
	return class_order * (len(class_order) == numCourses)
		



print('ANS->{}'.format(findOrder(4, [[1,0],[2,0],[3,1],[3,2]])))
print('ANS->{}'.format(findOrder(2, [[0,1]])))
print('ANS->{}'.format(findOrder(2, [[0,1], [1,0]])))
print('ANS->{}'.format(findOrder(7, [[2,1],[6,5],[6,3],[5,1],[5,0],[4,5]])))